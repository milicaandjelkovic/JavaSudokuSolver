...
public class TestClass extends BaseTestClass{
    ...
    public void generateNewGameTest() {
        ...
        game.newGame();
        Assert.assertFalse(Arrays.deepEquals(test.generateGameArray(game), tmp));
    }
    ...
    public void helpOnCheckBoxFalseTest() {
        game.setHelp(false);
        Assert.assertFalse(game.isHelp());
    }
    ...
    public void selectedNumberTest() {
        ...
        Assert.assertEquals(game.getNumber(x, y), game.getSelectedNumber());
    }
        ...
        for (int y = 0; y < 9; y++) {
            for (int x = 0; x < 9; x++) {
                if (game.isSelectedNumberCandidate(x, y) && game.isCheckValid(x, y)) {
                    game.setNumber(x, y, game.getSelectedNumber());
                    Assert.assertTrue(game.isCheckValid(x, y));
                }
            }
        }
    }
